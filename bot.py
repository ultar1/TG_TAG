...
async def translate_command(update: Update, context: ContextTypes.DEFAULT_TYPE, text_to_translate: str = None) -> None:
    ...
    if not text_to_translate:
        ...
        if len(parts) < 2:
            await update.message.reply_text("Format: language_code text\nExample: es Hello world")
            return
...
async def summarize_url(update: Update, context: ContextTypes.DEFAULT_TYPE, url: str) -> None:
    if not gemini_model: await update.message.reply_text("AI summarizer not configured. (Missing GEMINI_API_KEY)."); return
...
def main() -> None:
    ...
    application.add_handler(MessageHandler(filters.TEXT & filters.Regex("^Translate Text$"), lambda u,c: prompt_for_input(u,c,'awaiting_translation_text', "Format: lang_code text (e.g., es Hello)","Pressed 'Translate'")))
