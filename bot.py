...
async def start_ai_chat(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    await update.message.reply_text("You are now in a continuous chat with the AI. It will remember your conversation.\n\nSend your message, or press 'End Chat' to return to the main menu.", reply_markup=reply_markup)
...
async def translate_command(update: Update, context: ContextTypes.DEFAULT_TYPE, text_to_translate: str = None) -> None:
    ...
    if not text_to_translate:
        if len(context.args) < 2: await update.message.reply_text("Usage: /translate <language> <text>"); return
    ...
    try:
        ...
        logger.error(f"Translate API error: {e}"); await feedback.edit_text("Sorry, an error occurred.")
...
async def summarize_file_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        if replied_message.photo:
            ...
            response = await asyncio.to_thread(gemini_model.generate_content, ["Describe this image in detail.", {"mime_type": "image/jpeg", "data": await photo_file.download_as_bytearray()}])
...
async def generate_image(update: Update, context: ContextTypes.DEFAULT_TYPE, prompt: str) -> None:
    ...
    try:
        ...
        await context.bot.send_photo(update.effective_chat.id, base64.b64decode(response.json()["artifacts"][0]["base64"]), caption=f"Creation: `{prompt}`", parse_mode=ParseMode.MARKDOWN)
...
async def animate_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        photo_file = await update.message.reply_to_message.photo[-1].get_file()
        # The correct Stability AI API endpoint for image-to-video
        ...
        for _ in range(45):
            ...
            elif res.status_code != 202: # If it's not 'in progress', break and report error
                logger.error(f"Animation failed with status {res.status_code}: {res.text}")
                break
        await feedback.edit_text("Sorry, the animation timed out or failed.")
    except Exception as e:
        logger.error(f"Animate command error: {e}"); await feedback.edit_text(f"Sorry, an error occurred: {e}")
...
async def search_and_play_song(update: Update, context: ContextTypes.DEFAULT_TYPE, song_name: str) -> None:
    ...
    try:
        ...
        logger.error(f"Play search error: {e}"); await feedback.edit_text("An error occurred while searching.")
...
async def handle_play_confirmation(update: Update, context: ContextTypes.DEFAULT_TYPE, media_type: str) -> None:
    ...
    try:
        ...
        with open(file_path, 'rb') as file:
            if media_type == 'audio':
                await context.bot.send_audio(query.message.chat_id, file, title=info.get('title'), duration=info.get('duration'))
            else:
                await context.bot.send_video(query.message.chat_id, file, supports_streaming=True)
        await query.delete_message()
    except Exception as e:
        logger.error(f"Download error for {video_id}: {e}"); await query.edit_message_text("An error occurred during download.")
    finally:
...
async def download_content_from_url(update: Update, context: ContextTypes.DEFAULT_TYPE, content_url: str) -> None:
    feedback = await update.message.reply_text(f"Starting download...")
    temp_dir = os.path.join(DOWNLOAD_DIR, str(uuid.uuid4())); os.makedirs(temp_dir, exist_ok=True)
    try:
        ydl_opts = {'outtmpl': os.path.join(temp_dir, '%(title)s.%(ext)s'), 'noplaylist': True, 'quiet': True, 'http_headers': {'User-Agent': 'Mozilla/5.0'}, 'format': 'bestvideo[ext=mp4]+bestaudio[ext=m4a]/best[ext=mp4]/best'}
        ...
        await feedback.edit_text(f"Download complete. Uploading...")
        with open(downloaded_file, 'rb') as f:
            await context.bot.send_video(update.effective_chat.id, f, supports_streaming=True)
        await feedback.delete()
    except Exception as e:
        logger.error(f"Download error for {content_url}: {e}"); await feedback.edit_text("Download failed. The link may be private or invalid.")
    finally:
        if os.path.exists(temp_dir): shutil.rmtree(temp_dir)
...
async def record_user_message(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    # Handle continuous chat state first
    if state == 'continuous_chat':
        await gemini_command(update, context)
        return

    # Then, handle single-action states
...
def main() -> None:
    ...
    application.add_handler(CommandHandler("gemini", gemini_command)) # For single-shot replies
    ...
    application.add_handler(MessageHandler(filters.TEXT & filters.Regex("^Download Media$"), lambda u,c: prompt_for_input(u,c,'awaiting_download_url', "Send the link to download from.","Pressed 'Download'")))
    ...
    # Static info buttons that trigger a command
    ...
    # General message handler (must be last to catch text after prompts)
    ...
    if not RENDER_APP_NAME:
        ...
        application.run_webhook(listen="0.0.0.0", port=PORT, url_path=BOT_TOKEN, webhook_url=f"https://{RENDER_APP_NAME}.onrender.com/{BOT_TOKEN}")
